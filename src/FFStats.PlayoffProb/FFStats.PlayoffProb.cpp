...
void Calculate(const ApiStandings& api_standings, const ApiSchedule& api_schedule,
  ...
  // convert standings struct
  {
    ...
    for (const auto& record : api_standings.records) {
      ...
      for (const auto& h2h_record : record.h2h_records) {
        ...
        standings.records[idx].setHead2HeadRecord(temp_opponent,
                                                  h2h_record.win - h2h_record.loss);
      }
  ...
  // convert schedule struct
  {
    for (const auto& week : api_schedule.weeks) {
      ...
      for (const auto& game : week.games) {
        ...
        games.push_back(
            {TeamInfo::getTemporaryId(game.team1), TeamInfo::getTemporaryId(game.team2)});
      }
  ...
  auto playoff_probs =
      ffstats::playoffprob::PlayoffProbCalculator::calculate(standings, schedule);
  ...
  // convert playoff prob output
  {
    ...
    for (const auto playoff_prob : playoff_probs) {
      ...
      api_playoff_probs->playoff_probs[idx].excl_tiebreakers =
          playoff_prob.excl_tiebreakers;
      api_playoff_probs->playoff_probs[idx].incl_tiebreakers =
          playoff_prob.incl_tiebreakers;
