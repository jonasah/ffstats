...
class PlayoffProbCalculator {
 public:
  static std::vector<PlayoffProbability> calculate(const Standings& start_standings,
 ...
 private:
  ...
  static std::array<WeekResults, num_results> generateWeekResults(
  ...
  static void generateOutcomes(const Schedule& schedule,
                               const Standings& prev_week_standings, Analyzer& analyzer) {
